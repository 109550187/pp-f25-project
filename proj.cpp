...
Image box_filter_tiled(const Image& input, int kernel_size) {
    ...
    // 1. Parallelize the TILES, not the pixels. 
    // collapse(2) fuses the y and x loops so OpenMP has more blocks to distribute.
    ...
    for (int ty = 0; ty < input.height; ty += TILE_SIZE) {
        for (int tx = 0; tx < input.width; tx += TILE_SIZE) {
            ...
            // 2. Process the pixels INSIDE the tile
            // [NOTE FOR TEAMMATE]: SIMD Optimization starts here.
            // Replace these two loops with AVX2 intrinsics.
...
Image gaussian_filter_tiled(const Image& input, int kernel_size, float sigma) {
    ...
    // Keeping your original logic of generating kernel locally
    vector<float> kernel = create_gaussian_kernel(kernel_size, sigma);
                ...
                for (int x = tx; x < x_end; x++) {
                    ...
                    for (int ky = -radius; ky <= radius; ky++) {
                        for (int kx = -radius; kx <= radius; kx++) {
                            ...
                            // Note: standard array access is easier for SIMD than .at()
                            // but we keep .at() here to match your specific coding style.
                            int kernel_idx = (ky + radius) * kernel_size + (kx + radius);
                            sum += input.at(px, py) * kernel[kernel_idx];
                        }
                    }
